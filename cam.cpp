...
Camera::Camera(float width, float height, glm::vec3 _pos) {
    ...
    proj = glm::perspective(glm::radians(45.0f), (float)(width / height), 0.1f,
                            10.0f);
    // glm::mat4 i = glm::scale(glm::identity<glm::mat4>(), glm::vec3(0.5,
    // 0.5, 1.)); glm::mat4 mat = glm::transpose(glm::ortho<double>(4, 5,
    // -1.0, 1.0, 0.1, 10)); if (pos.z == 2.0) {
    //     proj = (glm::mat4)glm::orthoZO<double>(-1, 1, -1, 1, 0.1, 10);
    // } else {
    //     proj = glm::translate((glm::mat4)glm::orthoZO<double>(-1, 1, -1, 1,
    //     0.1, 10), glm::vec3(0.1, 0.1, -0.1));
    // }
    // proj = i * glm::shear((glm::mat4)glm::orthoZO(-1., 1., -1., 1., 0.1,
    // 100.), glm::vec3(0., 0., 0.),
