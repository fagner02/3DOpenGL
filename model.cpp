...
VAOBuffers Model::getModelBuffers(const char *name) {
    ...
    const aiScene *scene = importer.ReadFile(
        name, aiProcess_Triangulate | aiProcess_GenSmoothNormals |
    ...
    for (int i = 0; i < scene->mNumMeshes; i++) {
        ...
        for (int j = 0; j < scene->mMeshes[i]->mNumVertices; j++) {
            vertices.push_back(
                glm::vec3(mVertices[j].x, mVertices[j].y, mVertices[j].z) *
                0.001f);

            aiColor3D color(0.f, 0.f, 0.f);
            scene->mMaterials[scene->mMeshes[i]->mMaterialIndex]->Get(
                AI_MATKEY_COLOR_DIFFUSE, color);
            normals.push_back(
                glm::vec3(mNormals[j].x, mNormals[j].y, mNormals[j].z));
            if (uvIndex > -1)
                uvs.push_back(glm::vec2(textureCoords[uvIndex][j].x,
                                        textureCoords[uvIndex][j].y));
...
void Model::saveFile(std::string destfile, glm::mat4 transform) {
    ...
    for (int i = 0; i < scene->mNumMeshes; i++) {
        for (int j = 0; j < scene->mMeshes[i]->mNumVertices; j++) {
            glm::vec4 res =
                transform * glm::vec4(scene->mMeshes[i]->mVertices[j].x,
...
void Model::draw(int shaderProgram) {
    ...
    for (int i = 0; i < meshes.size(); i++) {
        glDrawElementsBaseVertex(
            GL_TRIANGLES, meshes[i].indexNum, GL_UNSIGNED_INT,
